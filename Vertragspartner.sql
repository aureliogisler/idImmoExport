...
1 / (NVL(utab2.rowCounter, 0) + 1) AS MandatoryPortionAmount
...
LEFT OUTER JOIN (SELECT COUNT(*) AS rowCounter, solidarmieter.partner_id FROM solidarmieter INNER JOIN mv ON mv.partner_id = solidarmieter.partner_id AND mv.mv = solidarmieter.mv WHERE NVL(mv.endedat, '31.12.9999') > SYSDATE GROUP BY solidarmieter.partner_id) utab2 ON utab2.Partner_id = TEMP10.partner_id
